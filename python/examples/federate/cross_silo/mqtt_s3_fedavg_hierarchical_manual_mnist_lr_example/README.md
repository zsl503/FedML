# FedML Octopus Example with MNIST + Logistic Regression + Manual Launch
...
Futhermore, this example covers the scenario where there you opt for launching each node in silos manually (in contrast to launching all nodes automatically using pdsh).
...
Here `dist_training_args` defines the distributed training hierarchy for each silo where:
  ...
  - `manual_launch` indicates whether to launch all nodes automatically (usign pdsh) or manually.
