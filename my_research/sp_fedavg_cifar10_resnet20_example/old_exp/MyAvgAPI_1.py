...
def save_model(model_params, round_idx, path_tag, pre_desc=None, post_desc=None):
    # 保存全局权重，作为实验
    if pre_desc is None:
        pre_desc = path_tag

    save_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)),'weight_lists', TIME,path_tag)
...
class MyTrainer_1(ModelTrainerCLS):
    ...
    def train(self, train_data, device, args):
        ...
        for epoch in range(args.epochs):
            ...
            weight_list.append(copy.deepcopy(self.model.state_dict()))
        return weight_list

class Client:
    def __init__(
        self, client_idx, local_training_data, local_test_data, local_sample_number, args, device, model_trainer:MyTrainer_1,
    ):
    ...
    def train(self, w_global, round_idx):
        ...
        weight_list = self.model_trainer.train(self.local_training_data, self.device, self.args)
        weights = self.model_trainer.get_model_params()
        for index, w in enumerate(weight_list):
            save_model(w, round_idx, f'c{self.client_idx}', post_desc=f'{index}')
        return weights
...
class MyAvgAPI_1(object):
    def __init__(self, args, device, dataset, model):
        ...
        self.model_trainer = MyTrainer_1(model, args)
    ...
    def train(self):
        ...
        for round_idx in range(self.args.comm_round):
            ...
            for idx, client in enumerate(self.client_list):
                ...
                w = client.train(copy.deepcopy(w_global), round_idx=round_idx)
    ...
    def _local_test_on_all_clients(self, round_idx):
        save_model(self.model_trainer.get_model_params(), round_idx, "server")
